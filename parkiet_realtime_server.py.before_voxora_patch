...
class ParkietWorker:
    ...
    def _handle_job(self, job: GenerationJob) -> None:
        ...
        try:
            ...
            text = normalize_text(job.text, preset["speaker"])
            ...
            print(
                f"Parkiet generation start voice={job.voice} seed={seed} text={text[:120]!r}",
                flush=True,
            )
            ...
            def on_chunk(
                ...
                audio_chunk_16k = resample_audio_chunk(
                    audio_chunk,
                    sample_rate,
                    OUTPUT_SAMPLE_RATE,
                )

                pcm_bytes = float_audio_to_pcm16_bytes(audio_chunk_16k)
...
def resample_audio_chunk(audio_chunk: np.ndarray, source_rate: int, target_rate: int) -> np.ndarray:
